...
<div class="scrollbar-hover" style=" background: url(images/backcolor/<%=backcolor%>.jpg) repeat-x;background-size:contain; max-height: 700px; overflow: scroll;">   
	<div style="margin:0 auto; margin-top:100px; width: 80%;"  align="center" >
								...
								success:function(data){
									if(data.code=1){
										...
										for(i in result2){
											...
											var oneprice = parseInt(price); //删除第一个字符,并且将字符串转数字
										...
										console.log("'#"+Name+"price'"+"-----"+"'"+allprice+"'");
										$("#price"+Name+"").html("此套餐的总价格为："+allprice);
						...
						success:function(data){
							...
							if(data.code==1){
								...
								for( i in result ){
									...
									strs+='<h4 class="modal-title" style="color:black;">'+pkBean.name+'的菜品添加极其查看</h4>';   //标题
									strs+='</div>';
								...
								'</div>';
									...
									strs+='<div class="modal-footer" >'+
										 '<div id="price'+pkBean.name+'" style="width:50%;margin:0 auto;">此套餐的总价格为'+pkBean.price+'</div>'+
				...
				<div class="modal fade" id="myLgModal_menuadd">
				    ...
				    <div class="modal-content">
				      ...
				      <div class="modal-footer" >
				        ...
				        <button type="button" class="btn btn-default" data-dismiss="modal" style="float: right;margin-left: 3%;">关闭窗口</button>
				        <button type="button" class="btn btn-primary" onclick="addMenu('formmenuadd');" style="float: right;margin-left: 3%;">确认添加</button>
				      	<button type="button" class="btn btn-primary" onclick="AIPrice();" style="float: right;">智能定价</button>
				      </div>
				    </div>
				  </div>
				</div>
		...
		<div style="width: 100%;float: left;">
			...
			<form class="form-horizontal" id="searchorder">
				<div class="form-group" style="width:70%; margin:0.5%;float:right;">
					<button type="button" class="btn btn-primary" onclick="showorder()" style="float:right;margin-left: 0.5%;">添加订单</button>
					<button type="button" class="btn btn-primary" onclick="showorder()" style="float:right;">查询</button>
				    <div class="col-sm-3" style="float:right;">
				 	...
				 	<label for="exampleInputAccount4" class="col-sm-2" style="float:right;">搜索一下</label>
				 </div>
			</form>
			<div class="scrollbar-hover" style="width:100%; height: 300px;background-size:contain;
					background: url(images/backcolor/<%=backcolor%>.jpg) repeat-x;
				...
				<!--订单order的字段：orderNo name tel time deskcount price source age  Scene  append is_xiadan-->
					<thead id="showorderThead">
					<tr>
						<th>ID</th>
						<th>订单客户</th>
						<th>客户电话</th>
					...
					<tbody id="showorderTbody">
					
					</tbody>
				</table>
				<script>//异步刷新出订单信息
				function AIPrice(){
					var thematerial=$("#mn_material").val();
					console.log("admin，1_2智能定价，thematerial==="+thematerial);
					if( thematerial=='' ){
						alert("菜品成分不能为空");
						return;
					}
					$.ajax({
						url:"backData/admin_2AIprice.jsp",
						type:"POST",
						data:$("#formmenuadd").serialize(),
						dataType:"JSON",
						success:function(data){
							console.log("data.ai_price==="+data.ai_price);
							console.log("data.map2==="+data.map2);
							$("#mn_price").attr("value",data.ai_price);
						}
					});
				}
				function showorder(){
					var way=document.getElementById("way").value;
					console.log("admin，1_3订单管理，way==="+way);
					if(way=="未下单用户"){
						$("#showorderTbody").html("后台正在进行用户是否下单的预测，请稍候...");
					}
					$.ajax({
						url:"backData/admin_3showorder.jsp",
						type:"POST",
						...
						success:function(data){
							if(data.code=="1"){
								...
								var orderStrs='';
								for(i in orderlist){
									//orderNo name tel time deskcount price source age  Scene  append is_xiadan
									orderStrs+='<tr>'+
										'<td>'+orderlist[i].orderNo+'</td>'+
										'<td>'+orderlist[i].name+'</td>'+
										'<td>'+orderlist[i].tel+'</td>'+
										'<td>'+orderlist[i].time+'</td>'+
										'<td>'+orderlist[i].source+'</td>'+
										'<td>'+orderlist[i].age+'</td>'+
										'<td>'+orderlist[i].Scene+'</td>'+
										'<td>'+orderlist[i].append+'</td>'+
										'<td>'+orderlist[i].deskcount+'</td>'+
										'<td>'+orderlist[i].price+'</td>'+
									'</tr>';
								}
								$("#showorderTbody").html(orderStrs);
							}else{
								var orderlist = data.data;
								console.log(orderlist[0].checkd);
								var orderH =
								'<tr>'+
								'<th>ID</th>'+
								'<th>订单客户</th>'+
		...
		<div style="width: 100%; float: left;margin-left: 1%;">
			...
			<div class="header" style="width: 100%;float:left;">
				<ul>
					<li class="index_title">销量统计</li>
				</ul>
			</div>
			<div class="scrollbar-hover" style="width:100%; max-height: 300px; background-size:contain;
				background: url(images/backcolor/<%=backcolor%>.jpg) repeat-x;float:left; overflow: scroll;">
				<ul class="nav nav-stacked" >
				<li class="active" style="float:left;  width: 33%;">
						<div class="header" style="width: 100%;float:left;">
							<ul>
								<li class="index_title">柱状图</li>
							</ul>
						</div>
			  			<a class="ahover" href="analysis_Img/bar_menu.html" target="_Blank" style="width:100%; font-size: 150%;">销量前十菜品</a>
			  			<a class="ahover" href="analysis_Img//bar_package.html" target="_Blank" style="width:100%; font-size: 150%;">销量前十套餐</a>
			  	</li>
			  	<li class="active" style="float:left; width: 33%;">
			  			<div class="header" style="width: 100%;float:left;">
							<ul>
								<li class="index_title">饼状图</li>
							</ul>
						</div>
			  			<a class="ahover" href="analysis_Img/pie_menu.html" target="_Blank" style="width:100%; font-size: 150%;">销量前十菜品</a>
			  			<a class="ahover" href="analysis_Img/pie_package.html" target="_Blank" style="width:100%; font-size: 150%;">销量前十套餐</a>
			  	</li>
			  	<li class="active" style="float:left; width: 33%;">
			  			<div class="header" style="width: 100%;float:left;">
							<ul>
								<li class="index_title">词云图</li>
							</ul>
						</div>
			  			<a class="ahover" href="analysis_Img/wordcloud_material.html" target="_Blank" style="width:100%; font-size: 150%;">菜品原料词云</a>
			  	</li>
				</ul>
			</div>
		</div>		
